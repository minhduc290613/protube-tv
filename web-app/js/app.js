...
const { SUPABASE_URL, SUPABASE_ANON_KEY, DEVICE_ID, DEFAULT_VIDEO_IDS, DEFAULT_PIN, } = APP_CONFIG;
...
// ------------------------------------------------------------
const ICON_HOME = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" ' +
...
const ICON_STAR = '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l2.9 6.6 7.1.6-5.4 4.7 ' +
...
const state = {
    ...
    remotePlaylistId: '', // nếu có điền, ưu tiên dùng cái này thay vì remoteVideoIdsRaw
    remoteCachedVideos: null, // ưu tiên cao nhất — lấy qua YouTube Data API chính thức (xem mục 3.4)
    disconnectAt: null,
    timerId: 0,
    keyBuffer: '',
...
const els = {
    ...
    contentEyebrow: document.getElementById('content-eyebrow'),
    ...
    screenOffline: document.getElementById('screen-offline'),
    loadingState: document.getElementById('loading-state'),
    emptyState: document.getElementById('empty-state'),
    contentBody: document.getElementById('content-body'),
    heroCard: document.getElementById('hero-card'),
    heroThumb: document.getElementById('hero-thumb'),
    heroTitle: document.getElementById('hero-title'),
    grid: document.getElementById('grid'),
    playerSlot: document.getElementById('ytplayer-slot'),
};
// ------------------------------------------------------------
...
async function supabaseGet(query) {
    ...
    if (!res.ok)
        throw new Error(`Supabase GET lỗi: ${res.status}`);
    return res.json();
}
async function supabasePatch(query, body) {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/${query}`, {
        method: 'PATCH',
        headers: {
            apikey: SUPABASE_ANON_KEY,
    ...
    if (!res.ok)
        throw new Error(`Supabase PATCH lỗi: ${res.status}`);
}
async function fetchDeviceConfig() {
    const rows = await supabaseGet(`devices?id=eq.${encodeURIComponent(DEVICE_ID)}&select=video_ids,playlist_id,cached_videos,cached_videos_updated_at,pin,disconnect_at`);
    return rows[0] || null;
}
// ------------------------------------------------------------
...
function parseVideoId(raw) {
    ...
    if (/^[a-zA-Z0-9_-]{11}$/.test(s))
        return s; // đã là mã video (11 ký tự)
    const m = s.match(/(?:v=|\/embed\/|youtu\.be\/|\/shorts\/|\/v\/)([a-zA-Z0-9_-]{11})/);
    return m ? m[1] : null;
}
function splitRawIds(raw) {
    return raw
        .split(/[\n,]/)
        .map((s) => s.trim())
        .filter(Boolean);
...
async function fetchVideosByRawList(rawList) {
    const ids = rawList.map(parseVideoId).filter((x) => !!x);
...
//     ăn nhất.
// ------------------------------------------------------------
async function fetchVideosByPlaylist(playlistId) {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/get-playlist?id=${encodeURIComponent(playlistId)}`, {
        headers: {
            apikey: SUPABASE_ANON_KEY,
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
        },
    ...
    if (!res.ok)
        throw new Error(`Edge Function lỗi: ${res.status}`);
    const data = await res.json();
    if (data.error)
...
function applyDisconnectAt(iso) {
    ...
    if (delay <= 0)
        ...
        state.timerId = setTimeout(showDisconnectOverlay, delay);
}
function scheduleDisconnect(minutes) {
    if (minutes === 0) {
...
function setActiveScreen(el) {
    [els.screenGrid, els.screenPlayer, els.screenOffline].forEach((s) => s.classList.remove('is-active'));
...
async function selectNavItem(index) {
...
function updateContentFocusVisual() {
    els.heroCard.classList.toggle('is-focused', state.area === 'content' && state.contentFocus === -1);
...
function onKeyDown(e) {
    ...
    if (state.mode === 'disconnected')
        return; // "mất wifi" thì mọi phím khác vô hiệu
...
function startConfigPolling() {
    setInterval(async () => {
        ...
        if (newTs !== state.disconnectAt)
            applyDisconnectAt(cfg.disconnect_at);
        ...
        const cachedChanged = JSON.stringify(cfg.cached_videos || null) !== JSON.stringify(state.remoteCachedVideos);
        if (videoIdsChanged)
            state.remoteVideoIdsRaw = cfg.video_ids;
        if (playlistChanged)
            state.remotePlaylistId = newPlaylistId;
        if (cachedChanged)
...
async function init() {
    ...
    renderSidebar();
    const cfg = await fetchDeviceConfig().catch(() => null);
    state.pin = (cfg === null || cfg === void 0 ? void 0 : cfg.pin) || DEFAULT_PIN;
    state.remoteVideoIdsRaw = (cfg === null || cfg === void 0 ? void 0 : cfg.video_ids) || DEFAULT_VIDEO_IDS;
    state.remotePlaylistId = (cfg === null || cfg === void 0 ? void 0 : cfg.playlist_id) || '';
    state.remoteCachedVideos = (cfg === null || cfg === void 0 ? void 0 : cfg.cached_videos) || null;
    applyDisconnectAt((cfg === null || cfg === void 0 ? void 0 : cfg.disconnect_at) || null);
