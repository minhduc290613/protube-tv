...
const { FIREBASE_PROJECT_ID, FIREBASE_API_KEY, DEVICE_ID, GET_PLAYLIST_FUNCTION_URL, DEFAULT_VIDEO_IDS, DEFAULT_PIN, } = APP_CONFIG;
...
const state = {
    ...
    remoteCachedVideos: null, // ưu tiên cao nhất — lấy qua YouTube Data API chính thức
    disconnectAt: null,
    timerId: 0,
    keyBuffer: '',
...
function fromFirestoreValue(fv) {
    ...
    if ('mapValue' in fv) {
        const out = {};
        for (const [k, val] of Object.entries(fv.mapValue.fields || {}))
...
async function firestoreGetDevice() {
    const url = `${FIRESTORE_BASE}/devices/${encodeURIComponent(DEVICE_ID)}?key=${FIREBASE_API_KEY}`;
    const res = await fetch(url);
    if (res.status === 404)
        return null;
    if (!res.ok)
        throw new Error(`Firestore GET lỗi: ${res.status}`);
    const doc = await res.json();
    return fromFirestoreDoc(doc);
}
async function firestorePatchDevice(fields) {
    ...
    if (!res.ok)
        throw new Error(`Firestore PATCH lỗi: ${res.status}`);
}
...
async function fetchVideosByPlaylist(playlistId) {
    if (!GET_PLAYLIST_FUNCTION_URL)
        throw new Error('Chưa cấu hình GET_PLAYLIST_FUNCTION_URL');
    const res = await fetch(`${GET_PLAYLIST_FUNCTION_URL}?id=${encodeURIComponent(playlistId)}`);
    if (!res.ok)
        throw new Error(`Cloud Function lỗi: ${res.status}`);
    const data = await res.json();
    if (data.error)
